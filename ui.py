...
def get_Ers_doc_based_on_topic(topic_list_string):
   ...
   if topic_list_string!="":
      if "Search" in topic_list_string:
            ...
            col="{ boards(ids: 6633501571) {columns { id title}}}"
            # Execute the query
...
if st.session_state.getrag !=False:
    ...
    # Accept user input
    if question := st.chat_input("What features does customer wants about AI Ers?"):
    ...
    if  st.session_state.messages != [] : 
        output=st.session_state.getrag.invoke({"input": question})  
